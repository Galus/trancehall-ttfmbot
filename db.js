...
Bans.statics.Add = function(id, name, cb){
    this.findOne({ _id: id }, function(err, data) {
        if (err) {
            cb(err, null);
        } else if (data) {
...
/* 
Bans.statics.IsBanned = function(id, cb) {
    this.findOne({ _id: id }, function(err, data) {
 	if (err) { cb(err, false); }
	else if (data) { cb(err, true); }
	else { cb(err, false); }
      if (err) {
            cb(err, false);
        } else if (data) {
...
*/
Bans.statics.testBan = function(id, cb) {
	this.findOne( {'_id': id}, function(err, banData){
		if(err) { console.log('we have an error.');}
		else if (banData) { console.log('we have banData. ' + id); 
		//Send ban signal with id to bot.js function to remove user from booth.
		cb(err, id);
		}
		//else { console.log('we dont know whats going on.'); }
		});
};

Bans.statics.IsBanned = function(id) {
    this.findOne({ '_id' : id }, function(err, cp){
	if(err) { return false; }
